...
public class Hero : MonoBehaviour
{
    ...
    private float m_currentTargetOffsetX = 1;
    ...
    private void Awake()
    {
        ...
        m_inputActions.Player.Move.performed += ctx => m_moveInput = ApplyDeadZone(ctx.ReadValue<Vector2>());
    ...
    private void FixedUpdate()
    {
        ...
        m_positionComposer.TargetOffset.y = Mathf.Lerp(m_positionComposer.TargetOffset.y, targetOffsetY, Time.deltaTime * smoothSpeed);
        m_positionComposer.TargetOffset.x = Mathf.Lerp(m_positionComposer.TargetOffset.x, m_currentTargetOffsetX, Time.deltaTime * smoothSpeed);
    ...
    {
        m_impulseSource.GenerateImpulseWithForce(-1f);
        DoHitStop().Forget();
    ...
    private async UniTaskVoid Recoil(bool isDirectionRight)
    {
        ...
        screenShakeDirection.x *= -recoilDirection;
        
        m_impulseSource.GenerateImpulseWithVelocity(screenShakeDirection * m_screenShakeForce);
        m_rigidbody2D.linearVelocity = new Vector2(recoilDirection * m_recoilBackForce, m_recoilUpForce);
        await UniTask.Delay(TimeSpan.FromSeconds(m_recoilDuration));

        m_isRecoiling = false;
    }
    
    public async UniTask DoHitStop(float duration = 0.05f, float scale = 0.05f)
    {
